...
#if defined(WOLFSSL_TLS13) && (defined(HAVE_ECC) || defined(HAVE_CURVE25519) ||                    \
                               defined(HAVE_CURVE448) || defined(HAVE_FFDHE_2048))
...
static struct group_info group_id_to_text[] = {{WOLFSSL_ECC_SECP160K1, "SECP160K1"},
...
#endif
static int
srvHandShakeCB(HandShakeInfo* info)
{
    (void)info;
    return 0;
}
...
static int
myHsDoneCb(WOLFSSL* ssl, void* user_ctx)
{
    (void)user_ctx;
    (void)ssl;
...
static void
err_sys_ex(int out, const char* msg)
{
    if (out == 1)
    { /* if server is running w/ -x flag, print error w/o exit */
...
TranslateReturnCode(int old, int sd)
{
    (void)sd;
...
static WC_INLINE int
PeekSeq(const char* buf, word32* seq)
{
    const char* c = buf + 3;

    if ((c[0] | c[1] | c[2] | c[3]) == 0)
    {
        *seq = ((word32)c[4] << 24) | ((word32)c[5] << 16) | ((word32)c[6] << 8) | (word32)c[7];
        return 1;
    }
 ...
 */
static int
TestEmbedSendTo(WOLFSSL* ssl, char* buf, int sz, void* ctx)
{
    ...
    sent =
        (int)sendto(sd, buf, (size_t)sz, 0, (const SOCKADDR*)&dtlsCtx->peer.sa, dtlsCtx->peer.sz);
...
int
ServerEchoData(SSL* ssl, int clientfd, int echoData, int block, size_t throughput)
{
    int ret = 0, err;
...
/* do a peek and check for "pending" */
            ...
            do
            {
                ...
                err = SSL_get_error(ssl, ret);
            } while (err == WC_NO_ERR_TRACE(WC_PENDING_E) || err == WOLFSSL_ERROR_WANT_READ ||
...
ServerWrite(WOLFSSL* ssl, const char* output, int outputLen)
{
    ...
    } while (err == WC_NO_ERR_TRACE(WC_PENDING_E) || err == WOLFSSL_ERROR_WANT_WRITE);
    if (ret != outputLen)
    {
        char buffer[WOLFSSL_MAX_ERROR_SZ];
...
static void
SetKeyShare(WOLFSSL* ssl, int onlyKeyShare, int useX25519, int useX448, int usePqc, char* pqcAlg)
{
    int ret;
 ...
 */
static int
server_srtp_test(WOLFSSL* ssl, func_args* args)
{
...
/* Note: this requires TSL1.3 (version >= 4) */
            ...
            if (NULL == myoptarg)
            {
                ...
                for (idx = 0;; ++idx)
                {
                    ...
                    {
                        if (XSTRCMP(group_id_to_text[j].name, wc_ecc_get_curve_name_from_id(id)) ==
                            0)
                        {
    ...
    if (wc_InitNetRandom(wnrConfigFile, NULL, 5000) != 0)
    {
        err_sys_ex(runWithErrors,
                   "can't load whitewood net random config "
                   "file");
    }
#endif
...
#ifdef WOLFSSL_WOLFSENTRY_HOOKS
    if (wolfsentry_setup(&wolfsentry, wolfsentry_config_path, WOLFSENTRY_ROUTE_FLAG_DIRECTION_IN) <
        0)
    {
        err_sys("unable to initialize wolfSentry");
    }

    if (wolfSSL_CTX_set_AcceptFilter(ctx,
...
#if defined(WOLFSSL_SNIFFER) && defined(WOLFSSL_STATIC_EPHEMERAL) && defined(WOLFSSL_PEM_TO_DER)
    ...
    ret = wolfSSL_CTX_set_ephemeral_key(ctx,
                                        WC_PK_TYPE_ECDH,
    ...
    )
    {
        ...
        {
            err_sys_ex(catastrophic,
                       "can't load alt private key file, "
                       "check file and run from wolfSSL home dir");
        }
    ...
    if (doCliCertCheck && (usePsk == 0 || usePskPlus) && useAnon == 0)
    {
        ...
        SSL_CTX_set_verify(
            ctx,
            WOLFSSL_VERIFY_PEER |
                (usePskPlus ? WOLFSSL_VERIFY_FAIL_EXCEPT_PSK : WOLFSSL_VERIFY_FAIL_IF_NO_PEER_CERT),
            (myVerifyAction == VERIFY_OVERRIDE_DATE_ERR || myVerifyAction == VERIFY_FORCE_FAIL)
                ? myVerify
                : NULL);
    ...
    if (!disableCRL)
    {
        ...
        if (wolfSSL_CTX_LoadCRL(ctx,
                                crlDir != NULL ? crlDir : crlPemDir,
                                WOLFSSL_FILETYPE_PEM,
                                crlFlags) != WOLFSSL_SUCCESS)
        {
    ...
    while (1)
    {
        ...
        if (resumeCount > 1)
        {
            if (dtlsUDP == 0)
            {
                ...
                clientfd =
                    accept(sockfd, (struct sockaddr*)&client_addr, (ACCEPT_THIRD_T)&client_len);
            }
            else
            {
...
#ifdef WOLFSSL_SEND_HRR_COOKIE
        if (hrrCookie == 1 && wolfSSL_send_hrr_cookie(ssl, NULL, 0) != WOLFSSL_SUCCESS)
        {
...
#define PMS_SZ 512                           /* ENCRYPT_LEN (see internal.h) */
        ...
        if (postHandAuth)
        {
            ...
            if (trustCert)
            {
                if (wolfSSL_trust_peer_cert(ssl, trustCert, WOLFSSL_FILETYPE_PEM) !=
                    WOLFSSL_SUCCESS)
                {
                    err_sys_ex(runWithErrors,
                               "can't load trusted peer cert "
                               "file");
                }
            }
#endif /* WOLFSSL_TRUST_PEER_CERT */
    ...
    (defined(HAVE_CERTIFICATE_STATUS_REQUEST) || defined(HAVE_CERTIFICATE_STATUS_REQUEST_V2))
        { /* scope start */
            ...
            {
                err_sys_ex(catastrophic,
                           "Failed to load any intermediates for "
                           "OCSP stapling test");
            }
        } /* scope end */
        ...
        if (trustedCaKeyId)
        {
            if (wolfSSL_UseTrustedCA(ssl, WOLFSSL_TRUSTED_CA_PRE_AGREED, NULL, 0) !=
                WOLFSSL_SUCCESS)
            {
...
#ifdef WOLFSSL_WOLFSENTRY_HOOKS
        {
            ...
            if (wolfsentry_store_endpoints(ssl,
                                           &client_addr,
                                           &local_addr,
    ...
    !defined(HAVE_SELFTEST)
        ...
        if (cipherList && !useDefCipherList && (!XSTRSTR(cipherList, ":")))
        {
                                                    ...
                                                    &requested_cipherFlags) == 0))
            {
                     ...
                     strcmp(cipherList, established_cipher_name_iana)))
                {
                    ...
                    {
                        err_sys_ex(catastrophic,
                                   "Unexpected mismatch between names of requested and established "
                                   "ciphers.");
                    }
...
#if defined(OPENSSL_EXTRA) || defined(HAVE_SECRET_CALLBACK)
        {
            ...
            {
                err_sys_ex(runWithErrors,
                           "error getting server random buffer "
                           "size");
            }
            else
            {
