...
/* ========================================================= */
/* QKDNetSim: REAL ETSI-014 SAE-KMS INTEGRATION              */
/* ========================================================= */
...
fetch_qkd_key_from_kms(char *out_key_id, byte *out_key_material)
{
    ...
    if (key_already_fetched)
    {
        ...
        {
            strcpy(out_key_id, cached_key_id);
        }
        if (out_key_material != cached_key_material)
        {
    ...
    strncpy(cached_key_id, id_start, id_len);
    cached_key_id[id_len] = '\0';
    char key_b64[256];
    size_t key_b64_len = key_end - key_start;
    strncpy(key_b64, key_start, key_b64_len);
...
EarlyData(WOLFSSL_CTX *ctx, WOLFSSL *ssl, const char *msg, int msgSz, char *buffer)
{
    ...
    WOLFSSL_ASYNC_WHILE_PENDING(ret = wolfSSL_write_early_data(ssl, msg, msgSz, &msgSz), ret <= 0);
    if (ret != msgSz)
    {
    ...
    while (loops--)
    {
        ...
        for (i = 0; i < times; i++)
        {
            ...
            else if (version >= 4)
            {
                if (!helloRetry)
                {
                    SetKeyShare(ssl, onlyKeyShare, useX25519, useX448, usePqc, pqcAlg, 1);
                }
                else
                {
...
#if defined(WOLFSSL_TLS13) && !defined(NO_SESSION_CACHE) && defined(WOLFSSL_EARLY_DATA)
        ...
        if (tx_buffer && rx_buffer)
        {
            ...
            if (ret == 0)
            {
                ...
                while (throughput > xfer_bytes)
                {
                    ...
                    /* Perform TX */
                    start = current_time(1);
                    WOLFSSL_ASYNC_WHILE_PENDING(ret = wolfSSL_write(ssl, tx_buffer, len), ret <= 0);
                    if (ret != len)
                    {
...
#endif
    printf("wolfSSL Client Benchmark " SIZE_FMT " bytes\n"
           "\tConnect %8.3f ms\n"
           ...
           conn_time * 1000,
           (double)tx_time * 1000,
           (double)throughput / tx_time / 1024 / 1024,
           (double)rx_time * 1000,
           (double)throughput / rx_time / 1024 / 1024);
...
SMTP_Shutdown(WOLFSSL *ssl, int wc_shutdown)
{
    ...
    WOLFSSL_ASYNC_WHILE_PENDING(ret = wolfSSL_read(ssl, tmpBuf, sizeof(tmpBuf) - 1), ret < 0);
    if (ret < 0)
    {
    ...
    {
        if (tcp_select(wolfSSL_get_fd(ssl), DEFAULT_TIMEOUT_SEC) == TEST_RECV_READY)
        {
...
ClientRead(WOLFSSL *ssl, char *reply, int replyLen, int mustRead, const char *str, int exitWithRet)
{
    ...
    do
    {
        ...
        if (mustRead && (err == WOLFSSL_ERROR_WANT_READ || err == WOLFSSL_ERROR_WANT_WRITE))
        {
    ...
    } while ((mustRead && err == WOLFSSL_ERROR_WANT_READ) || err == WOLFSSL_ERROR_WANT_WRITE
#ifdef WOLFSSL_ASYNC_CRYPT
             || err == WC_NO_ERR_TRACE(WC_PENDING_E)
#endif
             || err == WC_NO_ERR_TRACE(APP_DATA_READY));
...
showPeerPEM(WOLFSSL *ssl)
{
    ...
    if (peer)
    {
        ...
        {
            if (wolfSSL_BIO_set_fp(bioOut, stdout, BIO_NOCLOSE) != WOLFSSL_SUCCESS)
            {
...
#if defined(WOLFSSL_STATIC_MEMORY) && defined(WOLFSSL_STATIC_MEMORY_DEBUG_CALLBACK)
static void
ExampleDebugMemoryCb(size_t sz, int bucketSz, byte st, int type)
{
    switch (st)
    {
...
client_test(void *args)
{
                               ...
                               0)) != -1)
    {
        ...
        case 260:
            doDTLS = 1;
            dtlsUDP = 1;
            dtlsSrtpProfiles = myoptarg != NULL ? myoptarg : "SRTP_AES128_CM_SHA1_80";
        ...
        case 'F':
            ...
            if (maxFragment < WOLFSSL_MFL_MIN || maxFragment > WOLFSSL_MFL_MAX)
            {
                Usage();
                XEXIT_T(MY_EX_USAGE);
            }
...
#if defined(HAVE_CERTIFICATE_STATUS_REQUEST) || defined(HAVE_CERTIFICATE_STATUS_REQUEST_V2)
        {
            ...
            if (myoptargSz > 0 && XTOUPPER((unsigned char)myoptarg[myoptargSz - 1]) == 'M')
            {
                mustStaple = 1;
            }
        }
...
#if !defined(HAVE_AESGCM) && defined(NO_AES) && !(defined(HAVE_CHACHA) && defined(HAVE_POLY1305))
 ...
 * suites. With AES disabled we only offer PolyChacha suites. */
    ...
    if (usePqc)
    {
        if (version == CLIENT_DOWNGRADE_VERSION || version == EITHER_DOWNGRADE_VERSION)
        {
            LOG_ERROR("WARNING: If a TLS 1.3 connection is not negotiated, you "
    ...
    if (wc_LoadStaticMemory(&heap, memory, sizeof(memory), WOLFMEM_GENERAL, 1) != 0)
    {
    ...
    if (wolfSSL_CTX_load_static_memory(&ctx,
                                       NULL,
                                       memoryIO,
                                       sizeof(memoryIO),
    ...
    {
        if (wolfSSL_CTX_set_tlsext_use_srtp(ctx, dtlsSrtpProfiles) != 0)
        {
...
#ifdef WOLFSSL_WOLFSENTRY_HOOKS
    if (wolfsentry_setup(&wolfsentry, wolfsentry_config_path, WOLFSENTRY_ROUTE_FLAG_DIRECTION_OUT) <
        0)
    {
        err_sys("unable to initialize wolfSentry");
    }

    if (wolfSSL_CTX_set_ConnectFilter(ctx,
    ...
    if (useClientCert && !loadCertKeyIntoSSLObj)
    {
        ...
        if (0 /*wolfSSL_CTX_use_certificate_chain_file_format(ctx, ourCert, fileFormat) !=
            WOLFSSL_SUCCESS*/)
        {
            // wolfSSL_CTX_free(ctx);
            // ctx = NULL;
...
#elif !defined(TEST_LOAD_BUFFER)
        if (0 /*wolfSSL_CTX_use_PrivateKey_file(ctx, ourKey, fileFormat) != WOLFSSL_SUCCESS*/)
        {
            // wolfSSL_CTX_free(ctx);
            // ctx = NULL;
    ...
    !defined(NO_WOLFSSL_DIR)
        if (useCertFolder)
        {
            ...
            if (wolfSSL_X509_LOOKUP_ctrl(lookup,
                                         WOLFSSL_X509_L_ADD_DIR,
                                         caCertFolder,
                                         X509_FILETYPE_PEM,
...
/* load ecc verify too, echoserver uses it by default w/ ecc */
...
#elif !defined(TEST_LOAD_BUFFER)
            if (0 /*doPeerCheck != 0 && !customVerifyCert &&
            ...
            if (trustCert)
            {
                if (wolfSSL_CTX_trust_peer_cert(ctx, trustCert, WOLFSSL_FILETYPE_PEM) !=
                    WOLFSSL_SUCCESS)
                {
                    wolfSSL_CTX_free(ctx);
                    ctx = NULL;
    ...
    if (useX25519)
    {
        if (wolfSSL_CTX_UseSupportedCurve(ctx, WOLFSSL_ECC_X25519) != WOLFSSL_SUCCESS)
        {
    ...
    if (useX448)
    {
        if (wolfSSL_CTX_UseSupportedCurve(ctx, WOLFSSL_ECC_X448) != WOLFSSL_SUCCESS)
        {
...
#if defined(WOLFSSL_STATIC_MEMORY) && defined(DEBUG_WOLFSSL) && !defined(WOLFSSL_STATIC_MEMORY_LEAN)
...
#elif !defined(TEST_LOAD_BUFFER)
        if (0 /*wolfSSL_use_certificate_chain_file_format(ssl, ourCert, fileFormat) != WOLFSSL_SUCCESS*/)
        {
            // wolfSSL_CTX_free(ctx);
            // ctx = NULL;
            // err_sys("can't load client cert file, check file and run from"
...
#elif !defined(TEST_LOAD_BUFFER)
        if (0 /*wolfSSL_use_PrivateKey_file(ssl, ourKey, fileFormat) != WOLFSSL_SUCCESS*/)
        {
            // wolfSSL_CTX_free(ctx);
            // ctx = NULL;
            // err_sys("can't load client private key file, check file and run "
    ...
    {
        SetKeyShare(ssl, onlyKeyShare, useX25519, useX448, usePqc, pqcAlg, 0);
    }
    else
    {
        wolfSSL_NoKeyShares(ssl);
...
#define PMS_SZ 512                       /* ENCRYPT_LEN (see internal.h) */
    ...
    {
        if (version == 4 &&
            (statusRequest == OCSP_STAPLINGV2 || statusRequest == OCSP_STAPLINGV2_MULTI))
        {
        ...
        case OCSP_STAPLING:
            if (wolfSSL_UseOCSPStapling(ssl, WOLFSSL_CSR_OCSP, WOLFSSL_CSR_OCSP_USE_NONCE) !=
                WOLFSSL_SUCCESS)
            {
                wolfSSL_free(ssl);
                ssl = NULL;
        ...
        case OCSP_STAPLINGV2:
            if (wolfSSL_UseOCSPStaplingV2(ssl, WOLFSSL_CSR2_OCSP, WOLFSSL_CSR2_OCSP_USE_NONCE) !=
                WOLFSSL_SUCCESS)
            {
                wolfSSL_free(ssl);
                ssl = NULL;
        ...
        case OCSP_STAPLINGV2_MULTI:
            if (wolfSSL_UseOCSPStaplingV2(ssl, WOLFSSL_CSR2_OCSP_MULTI, 0) != WOLFSSL_SUCCESS)
            {
                wolfSSL_free(ssl);
                ssl = NULL;
                CloseSocket(sockfd);
    ...
    // zeroization
    // the handshake is done. the key is safe inside the TLS engine
    // wipe the raw QKD key from our temporary cache to prevent memory scraping
    memset(cached_key_material, 0, 32);
    memset(cached_key_id, 0, 64);
    key_already_fetched = 0;
    printf("[SECURITY] QKD cache successfully zeroized from RAM.\n");
    // -------------------------------------------------------------------
    ...
    !defined(HAVE_SELFTEST)
        ...
        {
            printf("ALPN accepted protocols list : %s\n", alpnList);
            wolfSSL_UseALPN(sslResume, alpnList, (word32)XSTRLEN(alpnList), alpn_opt);
        }
#endif
#ifdef HAVE_SECURE_RENEGOTIATION
        if (scr)
        ...
        if (alpnList != NULL)
        {
            ...
            err = wolfSSL_ALPN_GetProtocol(sslResume, &protocol_name, &protocol_nameSz);
            if (err == WOLFSSL_SUCCESS)
            {
                printf("Received ALPN protocol : %s (%d)\n", protocol_name, protocol_nameSz);
            }
...
#if defined(HAVE_ECC) && defined(FP_ECC) && defined(HAVE_THREAD_LS) && defined(HAVE_STACK_SIZE)
